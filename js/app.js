...
function loadCart() {
  try {
    ...
    // Descarta ids que ya no existen en el catálogo
    for (const id of Object.keys(data)) {
      if (!PRODUCTS.some(p => p.id === id)) delete data[id];
    }
    return data;
  } catch {
    return {};
...
function addToCart(id, qty = 1) {
  ...
  showToast(`${p ? p.nombre : "Producto"} agregado al pedido`);
    ...
    return `
      <div class="cart-item">
        ...
        <div class="cart-item__info">
          ...
          <span>${escapeHtml(p.presentacion)}</span>
          <div class="qty">
            <button data-dec="${id}" aria-label="Restar uno de ${escapeHtml(p.nombre)}">−</button>
            <output aria-label="Cantidad">${cart[id]}</output>
            <button data-inc="${id}" aria-label="Sumar uno de ${escapeHtml(p.nombre)}">+</button>
          </div>
        </div>
        <button class="cart-item__remove" data-del="${id}" aria-label="Quitar ${escapeHtml(p.nombre)} del pedido">
...
$("#checkoutBtn").addEventListener("click", () => {
  ...
  const lines = ids.map(id => {
    ...
    return `• ${cart[id]} × ${p.nombre} (${p.presentacion})${precio}`;
  });
  const msg =
